...
- **Visage API reference** — comprehensive Canvas, Frame, Widget, Theme, Dimension, PostEffect, and Event system documentation

## How It Works

The skill has two layers:

1. **Generic skill** (`SKILL.md`) — reusable patterns and API reference that apply to any JUCE+Visage project
...
### Option 1: Via the juce-dev Plugin (Recommended)

Install the juce-dev plugin, which includes this skill automatically:
...
Restart Claude Code. The skill is loaded automatically when you work on JUCE+Visage projects.
...
git clone --filter=blob:none --sparse \
  ...
  /tmp/gcm-sparse
cd /tmp/gcm-sparse
git sparse-checkout set plugins/juce-dev/skills/juce-visage
mkdir -p ~/.claude/skills/juce-visage
cp -r plugins/juce-dev/skills/juce-visage/* ~/.claude/skills/juce-visage/
```
...
Update the plugin to get the latest skill:

```bash
/plugin update juce-dev@generous-corp-marketplace
```
...
If you deleted the sparse checkout, re-run the standalone installation steps above.
...
1. Create `docs/juce-visage-notes.md` (Claude will offer to do this from the template in SKILL.md)
...
| Visage Canvas API | Visage API Reference: Canvas |
| Frame API & lifecycle | Visage API Reference: Frame |
| Color/Brush/Theme | Visage API Reference: Color, Brush & Theme |
| Widgets (Buttons, etc.) | Visage API Reference: Widget Classes |
| Migration from JUCE UI | Visage API Reference: Migration Patterns |
